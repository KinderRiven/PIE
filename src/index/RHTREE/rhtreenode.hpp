...
namespace RHTREE {
  ...
  inline uint8_t FETCH_SIG(const uint64_t slot) { 
    return slot >> SIG_BIT & 0xFF; 
  }
  ...
  struct RHTreeLock {
   public:
    ...
    RHTreeLock() {
      memset (this, 0, sizeof(RHTreeLock));
    }
   ...
   public:
    ...
    int try_wlock() {
      ...
      // try lock writer
      uint16_t expect = 0;
      if (writer_cnt.compare_exchange_weak (expect, 1, std::memory_order_acquire) == false) {
        return 1;
      }
    ...
    void wr_unlock () {
      writer_cnt.store ((uint16_t)0, std::memory_order_release);
    }

    void rd_unlock () {
      reader_cnt.fetch_sub (1, std::memory_order_release);
    }

    void BucketLock(int idx) {
      bool expect = false;
      while (!bucketlocks[idx].compare_exchange_strong (expect, 
                                                        true, 
  ...
  struct LeafNode : public Node {
   ...
   public:
    ...
    // operation
    status_code_t leaf_insert(const RHTREE_Key_t &key, const RHTREE_Value_t &value,
                              Allocator *allocator);

    status_code_t leaf_search(const RHTREE_Key_t &key, RHTREE_Value_t &value);

    status_code_t leaf_update(const RHTREE_Key_t &key, const RHTREE_Value_t &value,
                              Allocator *allocator);
  ...
  inline bool LeafNode::PrefixMatch(const RHTREE_Key_t &key, int *height, 
    ...
    if (leaf_height != 0 &&
        memcmp(prefix, key.Data(), leaf_height) != 0) {
      return false;
    }
  ...
  inline int 
  LeafNode::HashBucket::FindExist(const RHTREE_Key_t &key, uint8_t fp, 
                                  uint8_t cache) const {
    auto i = decltype(kSlotNumPerBucket){0};
    for (i = 0; i < kSlotNumPerBucket; ++i) {
      if ((FETCH_SIG(slots[i]) == fp) && 
          (FETCH_CACHE(slots[i]) == cache) &&
  ...
  inline int
  LeafNode::HashBucket::FindEmpty(uint8_t lbound, uint8_t rbound, 
  ...
  inline auto
  LeafNode::HashBucket::FindEmptyAndExist(const RHTREE_Key_t &key, uint8_t fp, 
                                          uint8_t cache, uint8_t lbound, 
