...
namespace PIE {
  ...
  class PIENVMAllocator : public Allocator {
   ...
   public:
    ...
    void Print() const override {
      ...
      auto aligned_proportion = static_cast<double>(
        alignedregion_used_.load(std::memory_order_relaxed)) / alignedregion_size_;

      // Print out basic information
      std::cout << "[NVMAllocator]\n";
      std::cout << "[Unaligned Region][Base:" << static_cast<void*>(unalignedregion_base_)<< "]" 
                << "[Usage: " << unalignedregion_used_.load(std::memory_order_relaxed)
                << "B" << "(" << std::setprecision(4) << unaligned_proportion * 100 << "%)]\n";
  ...
  inline void *PIENVMAllocator::AllocateInAligned(size_t size, size_t alignment) {
    ...
    auto ret = alignedregion_used_.fetch_add(alloc_size, std::memory_order_relaxed);

    // Check if there is enough space
    if (ret + alloc_size <= alignedregion_size_) {
      return reinterpret_cast<void *>(alignedregion_base_ + ret);
    } else {
      std::cerr << "[PIENVMAllocator: Aligned region has no enough space]" << std::endl;
      exit(1);
    }
  }
  ...
  inline void *PIENVMAllocator::AllocateInUnAligned(size_t size) {
    ...
    if (ret + size <= unalignedregion_size_) {
      ...
      std::cerr << "[PIENVMAllocator: UnAligned region has no enough space]" << std::endl;
      exit(1);
    }
  }
