...
public:
  ...
  virtual status_code_t Insert(const char *key, size_t key_len,
                               void *value) = 0;
  ...
  virtual status_code_t Search(const char *key, size_t key_len,
                               void **value) = 0;

  // Update specified key with given value.
  // If the key does not exist in current index, kNotFound would
  // be returned, otherwise return kOk if update success
  virtual status_code_t Update(const char *key, size_t key_len,
                               void *value) = 0;
  ...
  // occurs
  virtual status_code_t Upsert(const char *key, size_t key_len,
                               void *value) = 0;
  ...
  virtual status_code_t ScanCount(const char *startkey, size_t key_len,
                                  size_t count, void **vec) = 0;
